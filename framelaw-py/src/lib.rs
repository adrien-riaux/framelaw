...
use framelaw_core::checks::{
    Check, NotNull, UniqueValues, GreaterThanF64, LessThanF64, BetweenF64, MatchesRegex, OneOfString
};
...
impl PyCheck {
    fn into_check(self) -> Box<dyn Check> {
        match self.inner {
            ...
            CheckType::BetweenF64(a, b) => Box::new(BetweenF64 { min: a, max: b }),
            CheckType::MatchesRegex(s) => Box::new(MatchesRegex::new(&s).unwrap()),
            CheckType::OneOfString(v) => Box::new(OneOfString { allowed: v.into_iter().collect() }),
        }
    }
}
...
pub fn not_null() -> PyResult<PyCheck> {
    Ok(PyCheck { inner: CheckType::NotNull })
}

#[pyfunction]
pub fn unique_values() -> PyResult<PyCheck> {
    Ok(PyCheck { inner: CheckType::UniqueValues })
}

#[pyfunction]
pub fn gt(value: f64) -> PyResult<PyCheck> {
    Ok(PyCheck { inner: CheckType::GreaterThan(value) })
}

#[pyfunction]
pub fn lt(value: f64) -> PyResult<PyCheck> {
    Ok(PyCheck { inner: CheckType::LessThan(value) })
}

#[pyfunction]
pub fn between(min: f64, max: f64) -> PyResult<PyCheck> {
    Ok(PyCheck { inner: CheckType::BetweenF64(min, max) })
}

#[pyfunction]
pub fn matches_regex(pattern: String) -> PyResult<PyCheck> {
    Ok(PyCheck { inner: CheckType::MatchesRegex(pattern) })
}

#[pyfunction]
pub fn is_in(allowed: Vec<String>) -> PyResult<PyCheck> {
    Ok(PyCheck { inner: CheckType::OneOfString(allowed) })
}
...
impl PySchema {
    ...
    fn validate(&self, df: PyDataFrame) -> PyResult<()> {
        ...
        for (name, col) in &self.columns {
            let checks: Vec<Box<dyn Check>> = col.checks.iter().map(|c| c.clone().into_check()).collect();
        ...
        if report.is_valid {
            ...
            for f in report.failures {
                msgs.push(format!("Column {}: row {}, error: {}", f.column_name, f.row_index, f.message));
            }
            Err(PyValueError::new_err(format!("Validation failed:\n{}", msgs.join("\n"))))
        }
    }
}
